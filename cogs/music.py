...
class Song:
    ...
    def create_embed(self, ctx: commands.Context):
        ...
        embed = discord.Embed(timestamp=self.ctx.message.created_at, description='```css\n{0.source.title}\n```'.format(self), color=242424)
...
class Music(commands.Cog):
    ...
    async def leave(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.voice:
            return await ctx.send(f'{config.forbidden} Currently not connected to any voice channels.')
    ...
    async def skip(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send(f'{config.forbidden} There are no music playing currently.')
    ...
    async def play(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.send(f'{config.success}' + '"{}"'.format(str(source)) + " Song added to queue.")
