...
class Leveling(commands.Cog):
	...
	async def on_message(self, message):
		...
		if author_data[f"{message.author.id}_XP"] >= int((50 * (user_data[f"{message.guild.id}_LEVEL"] ** 2)) + (50 * user_data[f"{message.guild.id}_LEVEL"])):
		...
		if author_data[f"GLOBAL_XP"] >= int((50 * (user_data[f"GLOBAL_LEVEL"] ** 2)) + (50 * user_data[f"GLOBAL_LEVEL"])):
